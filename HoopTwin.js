(() => {
  ...
  function safeSplitCSV(line, expectedCols) {
    ...
    if (expectedCols && parts.length > expectedCols) {
      ...
      const out = parts
        .slice(0, needed)
        .concat([parts.slice(needed).join(",")]);
      return out;
    }
    return parts;
  }
  ...
  function mergeRowsById(allRows) {
    ...
    for (const r of allRows) {
      ...
      for (const cand of idFieldCandidates) {
        if (r[cand] && r[cand].trim()) {
          key = r[cand].trim();
          break;
        }
      }
      if (!key) {
        const idLike = Object.values(r).find((v) =>
          /^[a-z]{5,}[0-9]{2,}/i.test(v),
        );
  ...
  function buildKDTree(points, ids, depth = 0) {
    ...
    const node = {
      ...
      axis,
      left: buildKDTree(
        items.slice(0, mid).map((x) => x.p),
        items.slice(0, mid).map((x) => x.id),
        depth + 1,
      ),
      right: buildKDTree(
        items.slice(mid + 1).map((x) => x.p),
        items.slice(mid + 1).map((x) => x.id),
        depth + 1,
      ),
    };
    return node;
  ...
  function kdTreeKNN(root, queryPoint, k) {
    ...
    function search(node) {
      ...
      if (best.length < k || d < best[0].dist)
        pushCandidate({ id: node.id, dist: d, idx: node.idx });
  ...
  fileInput.addEventListener("change", async (ev) => {
    ...
    featurePreview.textContent = numericCols.length
      ? `Auto features: ${numericCols.join(", ")}`
      : "No numeric features detected";
    if (numericCols.length) {
      const built = buildNormalized(rawRows, numericCols);
      normData = built.norm;
      const ids = rawRows.map(
        (r) => r["Player-additional"] || r["Player"] || r["player"] || "",
      );
  ...
  function populatePlayerSelect() {
    ...
    for (const r of rawRows) {
      const display =
        r["Player"] || r["player"] || r["Player-additional"] || "(unknown)";
  ...
  function runAlgorithm(mode = "both") {
    ...
    if (!numericCols.length || !normData.length) {
      status.textContent = "Select numeric features first.";
      return;
    }
    const playerName = playerSelect.value;
    if (!playerName) {
      status.textContent = "Select a player.";
      return;
    }
    ...
    if (queryIdx === -1) {
      status.textContent = "Selected player not found in merged data.";
      return;
    }
  ...
  function renderTable(thead, tbody, items) {
    ...
    for (const it of items) {
      ...
      nameTd.textContent =
        r["Player"] || r["player"] || r["Player-additional"] || "";
      tr2.appendChild(nameTd);
      const dTd = document.createElement("td");
      dTd.textContent = it.dist !== undefined ? it.dist.toFixed(4) : "";
      tr2.appendChild(dTd);
  ...
  downloadNorm.addEventListener("click", () => {
    ...
    for (let i = 0; i < rawRows.length; i++) {
      const row = rawRows[i];
      const name =
        row["Player"] || row["player"] || row["Player-additional"] || "";
    ...
    a.href = url;
    a.download = "hooptwin_normalized_dataset.csv";
    a.click();
